...
for fold in range(10):
    os.system(f'CUDA_VISIBLE_DEVICES="1" fairseq-hydra-train task.data=../../../data/coswara_10folds_fold{fold} model.w2v_path=../../../outputs/2022-07-25/00-11-55/checkpoints/checkpoint_best.pt --config-dir examples/wav2vec/config/finetuning --config-name base_1h')
    os.system(f'CUDA_VISIBLE_DEVICES="1" python examples/wav2vec/eval_classifier.py data --labels label --input_file coswara_10folds_fold{fold} --workspace wav2vec2_covid')
